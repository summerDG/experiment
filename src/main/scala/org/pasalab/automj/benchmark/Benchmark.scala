 ...
 *
 * @param sparkSession An existing SQLContext.
 */
...
//  def this() = this(new MjSession(SparkContext.getOrCreate()))

  val resultsLocation =
    sparkSession.sqlContext.getAllConfs.getOrElse(
  ...
  val defaultHcFixMode = Variation("Mj execution mode", Seq("default", "one-round", "mixed-fix")) {
    ...
    case "mixed-fix" => sparkSession.sqlContext.setConf(MjConfigConst.EXECUTION_MODE, "mixed-fix")
  }
  val hcFixMode = Variation("Mj execution mode", Seq("one-round", "mixed-fix")) {
    case "one-round" => sparkSession.sqlContext.setConf(MjConfigConst.EXECUTION_MODE, "one-round")
    case "mixed-fix" => sparkSession.sqlContext.setConf(MjConfigConst.EXECUTION_MODE, "mixed-fix")
  }
  val defaultMode = Variation("Mj execution mode", Seq("default")) {
    case "default" => sparkSession.sqlContext.setConf(MjConfigConst.EXECUTION_MODE, "default")
  }
  val confMode = Variation("Mj execution mode", Seq("mixed-conf")) {
    case "mixed-conf" => sparkSession.sqlContext.setConf(MjConfigConst.EXECUTION_MODE, "mixed-conf")
  }
...
//  }
  ...
  def runExperiment(
    ...
    new ExperimentStatus(executionsToRun, includeBreakdown, iterations, variations, tags,
      timeout, resultLocation, sparkSession, allTables, currentConfiguration, forkThread = forkThread)
  }
...
//                forkThread=forkThread)
              if (q.name.contains("arbitrary-2") && setup.contains("one-round") && q.isInstanceOf[Query]) {
    ...
    def scheduleCpuCollection(fs: FS) = {
      logCollection = () => {
        ...
        try {
          val location = cpu.collectLogs(sparkSession, fs, timestamp)
    ...
    def cpuProfile = new Profile(sparkSession, sparkSession.read.json(getCpuLocation(timestamp)))

    def cpuProfileHtml(fs: FS) = {
      s"""
         |<h1>CPU Profile</h1>
         |<b>Permalink:</b> <tt>sparkSession.read.json("${getCpuLocation(timestamp)}")</tt></br>
    ...
    def getCurrentResults() = {
      val tbl = sparkSession.createDataFrame(currentResults)
    ...
    def getCurrentRuns() = {
      ...
      tbl.createOrReplaceGlobalTempView("currentRuns")
      tbl
    }
    ...
    def html: String = {
      ...
      s"""
         ...
         |<b>Permalink:</b> <tt>sparkSession.read.json("$resultPath")</tt><br/>
